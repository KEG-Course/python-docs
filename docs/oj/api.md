...
- API 异常处理顺序为 **401 > 403 > 400 > 429 > 404 > 500**
...
```
- 异常：400 参数错误 / 401 未登录 (Step 4) / 403 权限不足 / 404 题目不存在 / 429 提交频率超限
...
```
- 异常：400 用户名已存在 & 参数错误 / 401 未登录 (Step 4)
...
```
- 异常：401 用户未登录 / 403 用户无权限 / 500 服务器异常
...
```
- 异常：400 参数错误 / 401 用户未登录 / 403 用户无权限 / 500 服务器异常
